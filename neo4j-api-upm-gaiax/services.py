...
def create_service_offering(session, service_offering_id, name="Unknown", description="No Description",
                            provided_by="No provided"):
    session.run(
...
def create_data_resource(session, data_resource_id, name="Unknown", description="No Description",
                         produced_by="No provided"):
    session.run(
...
def create_nodes_and_relationships(session, data):
    credentials = data.get("data", {}).get("completeSD", {}).get("selfDescriptionCredential", {}).get(
